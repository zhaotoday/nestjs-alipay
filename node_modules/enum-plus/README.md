...
<p align="center">
  <a href="https://github.com/shijistar/enum-plus" target="blank">
    <img src="https://cdn.jsdelivr.net/gh/shijistar/enum-plus@v3.1.6/public/enum-plus.svg" width="240" alt="enum-plus" />
  </a>
</p>
...
⬇️ &nbsp;[Introduction](#introduction) | [Features](#user-content-features) | [Installation](#user-content-installation) | [Enum Initialization](#user-content-enum-initialization) | [API](#user-content-api) | [Static Methods](#user-content-static-methods) | [Global Configuration](#user-content-global-configuration) | [User Stories](#user-content-user-stories) | [Plugin System](#user-content-plugin-system) | [Localization](#user-content-localization) | [Extensibility](#user-content-extensibility) | [Naming Conflicts](#user-content-naming-conflicts) | [Best Practices](#user-content-best-practices) | [Compatibility](#user-content-compatibility) | [Q&A](#user-content-qa) | [Support](#user-content-support)&nbsp; ⬇️
...
>
> If the enum types become `any` after upgrading to v3.0, please see [here](#user-content-why-did-all-enum-types-become-any-after-upgrading-to-version-30).
...
<p align="center">
  <a href="https://cdn.jsdelivr.net/gh/shijistar/enum-plus@v3.1.6/public/usage-screenshot-high-v3.mp4" target="_blank">
    <img src="https://cdn.jsdelivr.net/gh/shijistar/enum-plus@v3.1.6/public/usage-screenshot-v3.gif" width="500" alt="usage video" />
  </a>
</p>
...
<details>
  ...
  - [Why do I need this library? TypeScript already has the built-in enums](#user-content-why-do-i-need-this-library-typescript-already-has-the-built-in-enums)
  - [Do I have to install TypeScript? What if my project is in JavaScript?](#user-content-do-i-have-to-install-typescript-what-if-my-project-is-in-javascript)
  - [How about the performance of this library?](#user-content-how-about-the-performance-of-this-library)
  - [It seems that TypeScript is going to deprecate enum?](#user-content-it-seems-that-typescript-is-going-to-deprecate-enum)
  - [I have a great idea and would like to contribute to this project. What should I do?](#user-content-i-have-a-great-idea-and-would-like-to-contribute-to-this-project-what-should-i-do)
...
<script src="https://cdn.jsdelivr.net/npm/enum-plus@v3.1.6/umd/enum-plus.min.js"></script>
<!-- Legacy version, ES2015 compatible -->
<script src="https://cdn.jsdelivr.net/npm/enum-plus@v3.1.6/umd/enum-plus-legacy.min.js"></script>
```
...
- [enum-plus.umd.min.js.gz](https://github.com/shijistar/enum-plus/releases/download/v3.1.6/enum-plus.umd.min.js.gz)
- [enum-plus.umd.tar.gz](https://github.com/shijistar/enum-plus/releases/download/v3.1.6/enum-plus.umd.tar.gz) (Full package with sourcemap)
- [enum-plus-legacy.umd.min.js.gz](https://github.com/shijistar/enum-plus/releases/download/v3.1.6/enum-plus-legacy.umd.min.js.gz)
- [enum-plus-legacy.umd.tar.gz](https://github.com/shijistar/enum-plus/releases/download/v3.1.6/enum-plus-legacy.umd.tar.gz) (Full package with sourcemap)
...
> Want to enable code completion when entering `label`? Please refer to the [Enable Code Intelligence for Enum Item Labels](#user-content--enable-code-intelligence-for-enum-item-labels) section for more details.
...
You can also use dynamic field mapping rules to adapt to various different data structures. Please refer to the [Custom Field Mapping](#user-content--custom-field-mapping-in-array-format-initialization) section for more details.
...
>
> You can pass in some optional configuration options to better control the behavior of the enum. Please refer to the [Enum Configuration Options](#user-content--enum-configuration-options) section for details.
...
Returns an array of all enum item `label`(s). If [localization](#user-content-localization) has been enabled, the localized texts of each enum item will be returned.
...
Gets the display name of an enum item according to its value or key. If [localization](#user-content-localization) is enabled, the localized text will be returned.
...
The display name of the whole enum collection. It's optional and can be set as the second parameter of the `Enum` method. This name can be either a plain string or a localization key to support internationalized text. Please refer to the [Localization](#user-content-localization) section for more details.
...
Set a global localization function for all enums. This function will be used to get the localized text for enum items and enum type names. Please refer to the [Localization](#user-content-localization) section for more details.
...
Extend the `Enum` objects with custom methods. More details can be found in the [Extensibility](#user-content-extensibility) section.
...
Install a plugin to extend the functionality of all enums. More details can be found in the [Plugin System](#user-content-plugin-system) section.
...
Take React and Ant Design as examples. Please refer to the [Supports Various Frontend Frameworks](#user-content--supports-various-frontend-frameworks) section for more examples.
...
Internationalization support. Set the `label` field to a localization key, so that it displays the corresponding text based on the current language environment. Please refer to the [Localization](#user-content-localization) section for more details.
...
![JSDoc](https://cdn.jsdelivr.net/gh/shijistar/enum-plus@v3.1.6/public/jsdoc-en.png)
...
In the [4. Array Format](#user-content-4-array-format) section, we know that enums can be created from dynamic data arrays. However, the field names in the real world may be different from the default `value`, `label`, and `key`. In such cases, you can pass in a custom option to map these to other field names.
...
To add localization support to `enum-plus`, the simplest way is to install the corresponding [i18n plugin](#user-content-plugin-system), such as `@enum-plus/plugin-i18next`, which automatically passes the values of the `label` and `name` fields to i18next for translation.
...
> Once you have completed this feature, it is recommended that you consider publishing it as an npm package and share it in the [Awesome Plugins](#user-content-awesome-plugins) section, so that others can benefit from your work. If you believe that this project is very general, you can also consider submitting it to the official [enum-plus](https://github.com/shijistar/enum-plus/tree/master/packages) plugin repository. For specific development rules, please refer to the [Plugin Development Guide](./docs/plugin-development.md).
...
> In fact, the entire [Plugin System](#user-content-plugin-system) and the `Enum.install` method are implemented using `Enum.extends` at the underlying level.
...
6. **Internationalization Architecture:** Plan for internationalization from the outset by leveraging the library's [localization](#user-content-localization) features. A well-designed internationalization architecture minimizes future refactoring and facilitates global scalability.
...
Thanks to the high flexibility of [Plugin System](#user-content-plugin-system), it is quite easy to extend new features for enum-plus. Depending on the `generality` and `dependencies` of the feature, you can choose one of the following three ways to contribute:
...
3. **Custom Plugin** - The new feature needs to depend on a `private` external dependency, or the field is relatively niche. It is recommended that you publish an npm package yourself and share your plugin link in the [Awesome Plugins](#user-content-awesome-plugins) section, which can still be shared with everyone.
